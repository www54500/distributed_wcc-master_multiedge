...
public class ChooseNextCommunityComputation extends AbstractComputation<
  ...
  public double computeWcc(
    ...
    CommunityAggregatorData commAggData = (CommunityAggregatorData) commAggMap.get(community);
  ...
  private double computeWccRemovalIncrement(Vertex<IntWritable, WccVertexData, NullWritable> vertex) {
    ...
    int d_out = vertex.getNumEdges() - d_in; // multi-edge
  ...
  private int getNumberOfEdgesToCommunity(
    ...
    for (Map.Entry<Writable, Writable> e : ncm.entrySet()) {
      ...
      // edit to suit the multi-edge
      int[] commEdges = (int[]) ((ArrayPrimitiveWritable) e.getValue()).get();
      if (commEdges[0] == destCommunity.get()) {result += commEdges[1];}
    }
    return result;
  }
  ...
  private Set<IntWritable> getCandidateCommunities(
    ...
    //candidates.remove(new IntWritable(vData.getCommunity()));
     ...
     //candidates.remove(new IntWritable(vData.getCommunity()));
     // edit to suit the multi-edge
     Set<IntWritable> candidates = new TreeSet();
     for (Map.Entry<Writable, Writable> e : ncm.entrySet()) {
      int[] commEdges = (int[]) ((ArrayPrimitiveWritable) e.getValue()).get();
      if (commEdges[0] != ISOLATED_COMMUNITY && commEdges[0] != vData.getCommunity()) {
        candidates.add(new IntWritable(commEdges[0]));
      }
    }
     return candidates;
  }
