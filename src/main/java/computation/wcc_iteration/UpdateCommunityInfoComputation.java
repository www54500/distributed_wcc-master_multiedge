...
public class UpdateCommunityInfoComputation extends AbstractComputation<
  ...
  private void updateNeighborCommunityMap(WccVertexData vData,
    ...
    for (TransferMessage tm : messages) {
      int countEdges = 0;
      IntWritable sourceId = new IntWritable(tm.getSourceId());
      MapWritable ncm = vData.getNeighborCommunityMap();
      if (ncm.containsKey(sourceId)) {
        int[] commEdges = (int[]) ((ArrayPrimitiveWritable) ncm.get(sourceId)).get();
        if (commEdges[0] == tm.getCommunity()) {
          countEdges = commEdges[1];
        }
      }
      vData.getNeighborCommunityMap().put(
          sourceId, new ArrayPrimitiveWritable(new int[]{tm.getCommunity(), countEdges + 1})); //0=community, 1=edgeNum
    }
  }
}
